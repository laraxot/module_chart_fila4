# 📚 Documentazione SaluteOra - Indice Consolidato DRY + KISS

## 🎯 Navigazione Rapida

### 🏛️ Core - Fondamenti

- [🎭 Filosofia](core/filosofia.md) - Principi e valori del progetto
- [🏗️ Architettura](core/architettura_tecnologica.md) - Struttura tecnica
- [📋 Progetto](core/progetto.md) - Overview completa
- [🎨 Presentazione](core/presentazione.md) - Materiali presentazione

### 🛠️ Development - Sviluppo

- [🚀 Getting Started](development/getting-started.md) - Primi passi
- [📏 Coding Standards](development/coding-standards.md) - Standard di codifica
- [🧪 Testing](development/testing.md) - Strategie di test
- [🐛 Debugging](development/debugging.md) - Risoluzione problemi

### 📦 Modules - Moduli

- [📊 Overview](modules/overview.md) - Panoramica moduli
- [⚙️ Xot](modules/xot/) - Modulo core framework
- [🏥 SaluteOra](modules/saluteora/) - Modulo principale
- [🎨 UI](modules/ui/) - Componenti interfaccia
- [👤 User](modules/user/) - Gestione utenti
- [🏢 Tenant](modules/tenant/) - Multi-tenancy

### 📖 Guides - Guide Pratiche

- [💾 Installazione](guides/installation.md) - Setup iniziale
- [⚙️ Configurazione](guides/configuration.md) - Configurazione sistema
- [🚀 Deployment](guides/deployment.md) - Deploy in produzione
- [🔄 Migration](guides/migration.md) - Migrazioni database

### 📋 Reference - Riferimenti Tecnici

- [🔌 API](reference/api.md) - Documentazione API
- [🗄️ Database](reference/database.md) - Schema database
- [⌨️ Commands](reference/commands.md) - Comandi Artisan
- [📊 Performance](reference/performance.md) - Metriche performance

### 🔧 Templates - Template Standardizzati

- [📄 Module README](templates/module-readme.md) - Template documentazione modulo
- [✨ Feature Doc](templates/feature-doc.md) - Template documentazione feature
- [🚨 Troubleshooting](templates/troubleshooting.md) - Template risoluzione problemi

## 🚀 Quick Start

### Per Sviluppatori

1. [💾 Installazione](guides/installation.md)
2. [⚙️ Configurazione](guides/configuration.md)
3. [📏 Coding Standards](development/coding-standards.md)
4. [🧪 Testing](development/testing.md)

### Per Amministratori

1. [🚀 Deployment](guides/deployment.md)
2. [🔧 Troubleshooting](troubleshooting/)
3. [📊 Monitoring](reference/performance.md)

## 📊 Statistiche Consolidamento

### ✅ Risultati DRY + KISS

- **Duplicazioni eliminate**: 13 cartelle `_docs` consolidate
- **File migrati**: 150+ file .txt convertiti in .md
- **Struttura semplificata**: Da caotica a logica gerarchica
- **Template standardizzati**: 3 template riutilizzabili
- **Naming convention**: 100% lowercase (eccetto README.md)

### 🎯 Benefici Ottenuti

- **DRY**: Una sola fonte di verità per ogni concetto
- **KISS**: Navigazione intuitiva e struttura logica
- **Manutenibilità**: Aggiornamenti centralizzati
- **Coerenza**: Standard uniformi in tutta la documentazione

## 🔍 Ricerca e Navigazione

### Per Argomento

- **Filament**: [Filament Best Practices](best-practices.md#filament)
- **PHPStan**: [PHPStan Guide](development/phpstan.md)
- **Traduzioni**: [Translation System](development/translations.md)
- **Migrazioni**: [Migration Guide](guides/migration.md)

### Per Modulo

- **Xot**: [Modules/Xot/docs/](../laravel/Modules/Xot/docs/)
- **SaluteOra**: [Modules/SaluteOra/docs/](../laravel/Modules/SaluteOra/docs/)
- **UI**: [Modules/UI/docs/](../laravel/Modules/UI/docs/)

## 🆘 Supporto

- **Problemi Comuni**: [FAQ](faq.md)
- **Troubleshooting**: [Troubleshooting Guide](troubleshooting/)
- **Best Practices**: [Best Practices](best-practices.md)
- **Standards**: [Coding Standards](development/coding-standards.md)

---

## 📝 Note di Ristrutturazione

**Data Consolidamento**: 2025-08-04  
**Principi Applicati**: DRY (Don't Repeat Yourself) + KISS (Keep It Simple, Stupid)  
**Backup Disponibile**: `backup-docs-20250804/`  
**Script Utilizzato**: `bashscripts/docs-consolidation.sh`

### 🔄 Prossimi Passi

1. Revisione contenuti in `docs/_integration/` dei moduli
2. Aggiornamento link interni
3. Eliminazione cartelle `_docs` dopo verifica
4. Implementazione sistema di validazione automatica

---

*Documentazione consolidata secondo principi DRY + KISS*  
*Ultimo aggiornamento: 2025-08-04 09:36*
