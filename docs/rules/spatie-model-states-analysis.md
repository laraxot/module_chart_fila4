...
## Confronto con l'Approccio di SaluteOra

Nonostante le capacità avanzate di `spatie/laravel-model-states`, SaluteOra utilizza una relazione `workflow()` personalizzata nella classe `Doctor` per i seguenti motivi:
...
2. **Integrazione con il Sistema**: Un workflow personalizzato si integra strettamente con altri moduli di SaluteOra (come notifiche, gestione tenant e permessi), permettendo una coerenza e personalizzazione superiore.
